...
std::vector<float> many_updates(float fund_value, std::vector<float> &returns, long n_periods) {
    // initialize arrays
...
std::vector<float> many_updates_gpu(float fund_value, std::vector<float> &returns, long n_periods) {
  // initialize arrays
  float totals[n_periods+1];
  totals[0] = fund_value;
...
void monte_carlo_historical(long n, float initial_capital, long n_periods, const std::string csv_fpath) {
    // TODO: write all to 1 data structure, then to 1 csv file so save on I/O for Monte Carlo?
...
void mc_simulations(std::atomic<long> &n_simulations,
  ...
  #pragma omp parallel for schedule(dynamic) num_threads(num_cpu_cores) //default(none) shared(n_blocks, block_size, n_simulations, max_n_simulations, n_periods, initial_capital, historical_returns, mc_data, final_values)
